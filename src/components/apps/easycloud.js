...
export class EasyCloud extends Component {
    constructor() {
        super();
        this.home_url = "https://easycloud.co.id";
        this.state = {
            url: "https://easycloud.co.id",
            display_url: "https://easycloud.co.id",
        };
    }

    componentDidMount() {
        let lastVisitedUrl = localStorage.getItem("EasyCloud-url");
        let lastDisplayedUrl = localStorage.getItem("EasyCloud-display-url");
    ...
    storeVisitedUrl = (url, display_url) => {
        localStorage.setItem("EasyCloud-url", url);
        localStorage.setItem("EasyCloud-display-url", display_url);
    };
    ...
    checkKey = (e) => {
        if (e.key === "Enter") {
            ...
            url = encodeURI(url);
            display_url = url;
            if (url.includes("easycloud.co.id")) {
                // 😅
                url = "https://easycloud.co.id";
                display_url = "https://easycloud.co.id";
            }
...
export const displayEasyCloud = () => {
    return <EasyCloud> </EasyCloud>;
};
